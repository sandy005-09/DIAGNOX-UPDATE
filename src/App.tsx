...
/* the file uses dynamic inline styles for gradients, dimensions, and animations.
   disabling the `react/no-inline-styles` rule prevents the "CSS inline styles
   should not be used" compile errors while keeping the existing layout logic.
*/
...
function HeroSection() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 h-[300px] pointer-events-none filter blur-[30px] animate-pulse ${theme === 'dark' ? 'hero-bottom-glow-dark' : 'hero-bottom-glow-light'}`}
      />
      ...
      >
        ...
        <div className="mb-[-264px] flex h-[349px] flex-col items-center justify-between px-[20px] py-0 text-center">
          ...
          <div className="h-[302px] w-[1000px]">
...
function FeaturesSection() {
  ...
  return (
    <section
      id="features"
      className={`relative flex items-center justify-center scroll-mt-[44px] h-[812px] ${theme === 'dark' ? 'bg-[#000000]' : 'bg-white'
        }`}
    >
      <div className="flex flex-col items-center w-[1440px] p-6 gap-[10px]">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="flex flex-col items-center text-center w-[1200px] gap-[36px]"
        >
          <h2
            className={`${FONTS.heading} text-[64px] leading-[100%] tracking-[0.5%] ${theme === 'dark' ? 'text-white' : 'text-black'}`}
          >
            Truck Fleet Monitoring & Diagnostics
          </h2>
...
function AboutUsSection() {
  ...
  return (
    <section id="about" className="relative overflow-hidden h-[1504px]">
      {/* First Part: How does DiagnoX work ? - 752px with silver background */}
      <div
        className={`relative h-[752px] ${theme === 'dark' ? 'bg-[#000000]' : 'bg-gray-50'}`}
      >
        <div className="mx-auto max-w-7xl px-6 py-24">
          ...
          >
            <h2 className={`mb-12 ${FONTS.subheading} text-[108px] leading-[1.04] tracking-[-3.786px] ${theme === 'dark' ? 'text-white/90' : 'text-black'
              ...
              How the DiagnoX  Platform Works
            </h2>
      ...
      <motion.div
        className="relative overflow-hidden h-[752px]"
        initial={{ opacity: 0 }}
        ...
        <div className="relative z-10 mx-auto h-full flex flex-col justify-between">
          ...
          <div className="flex justify-center">
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="flex items-center justify-center bg-[rgba(192,192,192,0.5)] backdrop-blur-sm border border-gray-400/40 shadow-lg w-full h-[72px]"
            >
              <h2 className={`${FONTS.subheading} text-[44px] leading-[1.1] tracking-[-0.24px] text-black`}>
                Benefits for Transport & Logistics Fleets
              </h2>
            </motion.div>
          </div>

          {/* Cards at bottom */}
          <div className="max-w-[1440px] mx-auto w-full px-6 pb-8 -mt-[264px]">
...
function WaitlistSection({ onSupportClick, onLearnMoreClick }: { onSupportClick: () => void; onLearnMoreClick: () => void }) {
  ...
  return (
    <section
      id="contact"
      className={`relative overflow-hidden min-h-screen flex flex-col ${theme === 'dark'
        ? 'bg-gradient-to-b from-[#0d0d0d] from-[73.676%] via-[#070d32] via-[102.02%] to-[#000d57] to-[113.83%]'
        : 'bg-gradient-to-b from-gray-50 via-blue-50 to-blue-100'
        }`}
    >
      {/* Glowing blue gradient at bottom - reduced glow */}
      <div
        className={`absolute bottom-0 left-0 right-0 h-[350px] pointer-events-none filter blur-[30px] animate-pulse ${theme === 'dark' ? 'contact-bottom-glow-dark' : 'contact-bottom-glow-light'}`}
      />
              ...
              >
                ...
                <div className="relative">
                  ...
                  <motion.div
                    ...
                    className={`absolute inset-0 rounded-full w-[80px] h-[80px] -top-[10px] -left-[10px] ${theme === 'dark'
                      ? 'bg-green-400/10'
                      : 'bg-green-500/10'
                      }`}
                  />
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col items-center gap-4">
                  ...
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <button
                      type="submit"
                      className={`group relative h-[44px] px-8 rounded-full ${FONTS.body} text-[14px] overflow-hidden transition-all duration-300 border-none ${theme === 'dark'
                        ? 'bg-white text-black'
                    ...
                    >
                      ...
                      <div
                        className={`absolute inset-0 transition-all duration-300 ${theme === 'dark'
                          ? 'opacity-0 group-hover:opacity-100 overlay-dark'
                          : 'opacity-100 group-hover:opacity-0 overlay-light'
                          }`}
                      />
                      ...
                      <div
                        className={`absolute inset-0 transition-all duration-300 ${theme === 'dark'
                          ? 'opacity-0'
                          : 'opacity-0 group-hover:opacity-100 bg-white rounded-full border-[0.65px] border-black'
                          }`}
                      />
                    </button>
                  </motion.div>
